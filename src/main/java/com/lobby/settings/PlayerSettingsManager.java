...
public class PlayerSettingsManager {
    ...
    private PlayerSettings loadSettingsFromDatabase(final UUID playerUuid) {
        final String query = "SELECT private_messages, group_requests, visibility, ui_sounds, particles, music, "
                + "clan_notifications, system_notifications, language FROM player_settings WHERE player_uuid = ?";
        ...
        try (Connection connection = databaseManager.getConnection();
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                if (resultSet.next()) {
                    ...
                    return new PlayerSettings(privateMessages, groupRequests, visibility, uiSounds,
                            particles, music, clanNotifications, systemNotifications, language);
                }
            }
    ...
    private void saveSettings(final UUID playerUuid, final PlayerSettings settings) {
        ...
        if (databaseManager.getDatabaseType() == DatabaseType.MYSQL) {
            sql = """
                    INSERT INTO player_settings (player_uuid, private_messages, group_requests, visibility, ui_sounds, particles, music, clan_notifications, system_notifications, language)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        try (Connection connection = databaseManager.getConnection();
            ...
            statement.setString(3, settings.getGroupRequestSetting().name());
            statement.setString(4, settings.getVisibilitySetting().name());
            statement.setBoolean(5, settings.isUiSounds());
            statement.setBoolean(6, settings.isParticles());
            statement.setBoolean(7, settings.isMusic());
            statement.setBoolean(8, settings.isClanNotifications());
            statement.setBoolean(9, settings.isSystemNotifications());
            statement.setString(10, settings.getLanguage().toLowerCase(Locale.ROOT));
